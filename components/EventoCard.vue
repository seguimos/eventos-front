<template>
  <v-card class="rounded-sm">
    <v-img
      ...
      :aspect-ratio="16/9"
      :alt="'image_' + evento.toLowerCase().replace(' ', '_')"
    />
    <v-card-title
      class="pt-1"
    >
    ...
    <v-card-subtitle class="py-0">
      <strong>Fecha:</strong> {{ date }} - {{ time }}Hrs.
      <div v-if="direccion">
  ...
  computed: {
    date () {
      return this.dateTimeStart
        ? new Date(this.dateTimeStart).toLocaleDateString('es', this.dateOptions)
        : ''
    },
    time () {
      return this.dateTimeStart.split(' ')[1]?.slice(0, -3)
    }
  }
}
</script>
